        ...
        <div>
            ...
            <div class="text-sm flex items-center gap-x-4 mb-2">
                ...
                <button @click="submitForm3()" :disabled="form3.processing"
                    class="border px-2 py-1 text-sm rounded-md bg-primary text-white">
            ...
            <div v-else>
                ...
                <div class="bg-white text-sm flex items-center justify-between border rounded-md px-2 py-2 mb-6">
                    ...
                    <button @click="openModal()" class="border rounded-md px-4 py-1">{{ $t('Update to version') + ' ' +
                        getValueByKey('available_version') }}</button>
                </div>
            </div>
            ...
            <div v-else class="grid grid-cols-2 gap-x-4 gap-y-1">
                <div v-for="addon in rows.data"
                    class="bg-white text-sm flex items-center justify-between border rounded-md px-2 py-2">
                    <div class="flex items-center gap-x-2">
                    ...
                    <button v-if="getValueByKey('is_update_available') == 0" @click="updateAddon(addon)"
                        class="border rounded-md px-4 py-1">Update</button>
            ...
            <div class="grid grid-cols-1 gap-x-6 gap-y-4">
                ...
                <form @submit.prevent="submitForm()">
                    <div>
                        ...
                        <h4 class="text-sm">{{ $t(`A new version is available. Would you like to proceed with updating
                            the module?`) }}</h4>
                    </div>
                    <div class="mt-3 border-t pt-5 flex">
                        <button type="button" @click.self="isUpdateModal = false"
        ...
        <Modal :isOpen=isOpenUpdateModal>
            <div class="grid grid-cols-1 gap-x-6 gap-y-4">
                ...
                <form @submit.prevent="submitForm2()">
                    ...
                    <div class="mt-3 border-t pt-5 flex">
                        <button type="button" @click.self="isOpenUpdateModal = false"
                            class="inline-flex justify-center rounded-md border border-transparent bg-slate-50 px-4 py-2 text-sm text-slate-500 hover:bg-slate-200 focus:outline-none focus-visible:ring-2 focus-visible:ring-blue-500 focus-visible:ring-offset-2 mr-4">{{
                                $t('No, not now') }}</button>
                        <button
