    ...
    <div class="grid md:grid-cols-3 gap-x-5 gap-y-4">
        <div v-for="addon in rows.data" :key="addon.id" class="bg-white rounded-[0.5rem] shadow-md text-sm space-y-3">
            ...
            <div class="flex justify-between items-center px-4 pb-4 text-xs">
                <div>
                    <span v-if="addon.status == 0" class=" py-1 px-3 rounded-md text-slate-600 bg-red-600">{{
                        $t('Not installed') }}</span>
                    <span v-else-if="addon.status == 1 && addon.update_available == 0"
                        class=" py-1 px-3 rounded-md text-slate-600 bg-slate-50">{{ $t('Installed') }}</span>
                    <Link v-else-if="addon.status == 1 && addon.update_available == 1" href="/admin/updates"
                        class="py-1 px-3 rounded-md text-slate-600 bg-green-600">{{ $t('Update available') }}
                    </Link>
                </div>
                <div>
                    <button v-if="addon.status == 0" @click="setupInstallModal(addon, addon.license)"
                        class="rounded-full border-2 w-full px-10 py-2 hover:border-secondary hover:bg-secondary hover:text-white text-secondary">{{
                            $t('Install') }}</button>
                    <button v-else @click="setupAddon(addon)"
                        class="rounded-full border-2 w-full px-10 py-2 hover:border-secondary hover:bg-secondary hover:text-white text-secondary">{{
                            $t('Setup') }}</button>
                </div>
            </div>
            ...
            <form @submit.prevent="submitForm()">
                <div class="grid gap-x-6 gap-y-4 sm:grid-cols-2">
                    <div v-for="input in modalInputs" :class="input.class">
                        <FormInput v-if="input.element == 'input'" v-model="form2.settings[input.name]"
                            :error="form2.errors[`settings.${input.name}`]" :name="$t(input.label)" :type="input.type"
                            :class="input.class" />
                        ...
                        <FormSelect v-if="input.element == 'select'" v-model="form2.settings[input.name]"
                            :name="$t(input.label)" :type="'text'" :options="input.options"
                            :error="form2.errors[`settings.${input.name}`]" :class="input.class" />
                        <FormTextArea v-if="input.element == 'textarea'" v-model="form2.settings[input.name]"
                            :error="form2.errors[`settings.${input.name}`]" :name="$t(input.label)"
                            :class="input.class" />
                    ...
                    <div v-if="modalTitle == 'Embedded Signup'"
                        class="bg-orange-100 p-2 rounded-md shadow-sm mb-1 col-span-2">
                        ...
                        <span>
                            <p class="text-sm leading-6 break-all">{{ $t('Callback URL') }}: {{ currentURL +
                                '/webhook/waba' }}</p>
                            <p class="text-sm leading-6 break-all">{{ $t('Verify token') }}: {{
                                getValueByKey('whatsapp_callback_token') }}</p>
                        </span>
                    </div>
                </div>
        ...
        <div class="grid grid-cols-1 gap-x-6 gap-y-4">
            ...
            <form @submit.prevent="submitForm3()">
                <div class="grid gap-x-6 sm:grid-cols-2">
                    ...
                    <FormInput v-model="form3.purchase_code" :error="form3.errors.purchase_code" :name="''"
                        :type="'text'" :class="'col-span-2'" />
                </div>
                <div class="mt-5 border-t pt-5 flex">
                    <button type="button" @click.self="isOpenInstallModal = false"
                        class="inline-flex justify-center rounded-md border border-transparent bg-slate-50 px-4 py-2 text-sm text-slate-500 hover:bg-slate-200 focus:outline-none focus-visible:ring-2 focus-visible:ring-blue-500 focus-visible:ring-offset-2 mr-4">{{
                            $t('Cancel') }}</button>
                    <button
                        ...
                        <svg v-if="form3.processing" xmlns="http://www.w3.org/2000/svg" width="20" height="20"
                            viewBox="0 0 24 24">
                            <path fill="currentColor"
                                d="M12 2A10 10 0 1 0 22 12A10 10 0 0 0 12 2Zm0 18a8 8 0 1 1 8-8A8 8 0 0 1 12 20Z"
                                opacity=".5" />
