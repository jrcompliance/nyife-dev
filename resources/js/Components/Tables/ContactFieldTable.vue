...
<template>
    <div>
        <div class="bg-slate-100 md:bg-slate-50 rounded-[0.5rem]">
            <table v-if="rows.data?.length > 0" class="w-full">
                <TableHeader>
                    <TableHeaderRow>
                        <TableHeaderRowItem :position="'first'" class="pl-2 py-2 pb-2 ">{{ $t('Input field name') }}
                        </TableHeaderRowItem>
                ...
                <draggable tag="tbody" :list="rows.data" handle=".handle" item-key="uuid" @update="onDragEnd">
                    <template #item="{ element, index }">
                        <tr class="hover:bg-slate-50 md:border-b-0 md:border-t border-[#d1d5db] text-sm">
                            ...
                            <td class="pl-2 py-2 pb-2 capitalize">
                                ...
                                <span v-if="element.type === 'input'"
                                    class="bg-slate-200 px-2 py-1 rounded-lg text-xs capitalize">{{
                                        $t(capitalizeFirstLetter(element.value)) }}</span>
                            </td>
                            <td class="pl-2 py-2 pb-2 capitalize">{{ element.required === 0 ? $t('No') : $t('Yes') }}
                            </td>
                            <td class="pr-4 float-right">
                                <Dropdown :align="'right'" class="mt-2">
                                    ...
                                    <template #items>
                                        <DropdownItemGroup>
                                            <DropdownItem as="button" @click="edit(element.uuid)">{{ $t('Edit') }}
                                            </DropdownItem>
                                            <DropdownItem as="button" @click="openAlert(element.uuid)">{{ $t('Delete')
                                                }}</DropdownItem>
                                        </DropdownItemGroup>
                                    </template>
                                </Dropdown>
