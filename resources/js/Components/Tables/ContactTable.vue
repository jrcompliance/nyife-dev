...
<template>
        ...
        <div class="flex items-center gap-3">
            <label @click="toggleAllCheckboxes($event)" class="cursor-pointer flex items-center gap-2 group">
                <div class="w-5 h-5 border-2 rounded-md flex items-center justify-center transition-all"
                    ...
                    <svg v-if="bulkCheckbox" class="w-3.5 h-3.5 text-white" fill="none" stroke="currentColor"
                        viewBox="0 0 24 24">
    ...
    <div class="border-b border-gray-100">
        <div class="flex">
            <Link href="/contacts" class="flex-1 py-4 text-center text-sm font-medium transition-all relative"
                :class="$page.url.startsWith('/contacts') ? 'text-[#ff5100]' : 'text-gray-500 hover:text-gray-700'">
            <span>{{ $t('All contacts') }}</span>
            <div v-if="$page.url.startsWith('/contacts')" class="absolute bottom-0 left-0 right-0 h-0.5 bg-[#ff5100]">
            </div>
            </Link>
            <Link href="/contact-groups" class="flex-1 py-4 text-center text-sm font-medium transition-all relative"
                :class="$page.url.startsWith('/contact-groups') ? 'text-[#ff5100]' : 'text-gray-500 hover:text-gray-700'">
            <span>{{ $t('Groups') }}</span>
            <div v-if="$page.url.startsWith('/contact-groups')"
                class="absolute bottom-0 left-0 right-0 h-0.5 bg-[#ff5100]">
            </div>
            </Link>
        </div>
    </div>
        ...
        <div v-if="type === 'contact'" v-for="(contact, index) in rows.data" :key="index" @click="getRow(contact.uuid)"
            ...
            <label @click.stop="toggleCheckbox(contact.uuid)" class="cursor-pointer flex-shrink-0">
                <div class="w-5 h-5 border-2 rounded-md flex items-center justify-center transition-all"
                    ...
                    <svg v-if="contact.isChecked" class="w-3.5 h-3.5 text-white" fill="none" stroke="currentColor"
                        viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="3" d="M5 13l4 4L19 7"></path>
                    </svg>
                </div>
            ...
            <div class="flex-shrink-0">
                <img v-if="contact.avatar" class="rounded-full h-12 w-12 object-cover ring-2 ring-gray-100"
                    :src="contact.avatar">
                <div v-else
                    class="rounded-full bg-gradient-to-br from-[#ff5100] to-orange-400 text-white flex justify-center items-center h-12 w-12 font-semibold text-lg">
            ...
            <div class="flex-shrink-0">
                <svg v-if="contact.is_favorite" xmlns="http://www.w3.org/2000/svg" width="20" height="20"
                    viewBox="0 0 24 24">
                    <path fill="#ff5100"
        ...
        <div v-else-if="type === 'group'" v-for="(row, key) in rows.data" :key="key" @click="getRow(row.uuid)"
            ...
            <label @click.stop="toggleCheckbox(row.uuid)" class="cursor-pointer flex-shrink-0">
                <div class="w-5 h-5 border-2 rounded-md flex items-center justify-center transition-all"
                    ...
                    <svg v-if="row.isChecked" class="w-3.5 h-3.5 text-white" fill="none" stroke="currentColor"
                        viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="3" d="M5 13l4 4L19 7"></path>
                    </svg>
                </div>
            ...
            <div class="flex-shrink-0">
                <div
                    class="rounded-full bg-gradient-to-br from-[#ff5100] to-orange-400 text-white flex justify-center items-center h-12 w-12 capitalize font-semibold text-lg">
