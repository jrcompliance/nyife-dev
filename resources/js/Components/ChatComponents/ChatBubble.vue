...
<template>
    <div class="rounded-lg my-1 p-2 text-sm flex flex-col relative"
        ...
        <div>
            ...
            <div v-if="JSON.parse(content.metadata).type === 'unsupported'" class="max-w-[300px]">
                ...
                <p class="normal-case whitespace-pre-wrap">{{ $t('Error code') }} : {{
                    JSON.parse(content.metadata).errors[0].code }}</p>
                <p class="normal-case whitespace-pre-wrap">{{ JSON.parse(content.metadata).errors[0].error_data.details
                    }}</p>
            </div>
            ...
            <div v-if="JSON.parse(content.metadata).type === 'interactive'" class="max-w-[300px]">
                <div v-if="JSON.parse(content.metadata).interactive.type == 'button_reply'">
                    <p class="normal-case whitespace-pre-wrap">{{
                        JSON.parse(content.metadata).interactive?.button_reply?.title }}</p>
                </div>
                <p v-if="JSON.parse(content.metadata).interactive.type == 'list_reply'"
                    class="normal-case whitespace-pre-wrap">{{
                        JSON.parse(content.metadata).interactive?.list_reply?.title }}</p>
                <p v-if="JSON.parse(content.metadata).interactive.type == 'list_reply'"
                    class="normal-case whitespace-pre-wrap">{{
                        JSON.parse(content.metadata).interactive?.list_reply?.description }}</p>
            </div>
            ...
            <div v-else-if="JSON.parse(content.metadata).type === 'image'">
                ...
                <div v-if="JSON.parse(content.metadata).image?.caption" class="max-w-[300px]">{{
                    JSON.parse(content.metadata).image?.caption }}</div>
                <div v-if="JSON.parse(content.metadata)?.buttons"
                    class="mr-auto text-sm text-[#00a5f4] flex flex-col relative max-w-[25em]">
                    <div v-for="(item, index) in JSON.parse(content.metadata)?.buttons" :key="index"
                        class="flex justify-center items-center space-x-2 rounded-lg bg-white h-10 my-[0.1em]">
                ...
                <div class="relative w-[300px]">
                    <div class="flex space-x-2 w-full h-1/3 bg-white opacity-90 pt-2">
                        <div>
                            ...
                            <svg v-if="getExtension(content?.media?.type) === 'XLSX' || getExtension(content?.media?.type) === 'XLS'"
                            ...
                            <svg v-if="getExtension(content?.media?.type) === 'DOC' || getExtension(content?.media?.type) === 'DOCX'"
                                xmlns="http://www.w3.org/2000/svg" width="36" height="36" viewBox="0 0 32 32">
                                <defs>
                            ...
                            <svg v-if="getExtension(content?.media?.type) === 'PPT' || getExtension(content?.media?.type) === 'PPTX'"
                                xmlns="http://www.w3.org/2000/svg" width="36" height="36" viewBox="0 0 32 32">
                                <defs>
            ...
            <div v-else-if="JSON.parse(content.metadata).type === 'video'">
                ...
                <div v-if="JSON.parse(content.metadata).video?.caption" class="max-w-[300px]">{{
                    JSON.parse(content.metadata).video?.caption }}</div>
                <div v-if="JSON.parse(content.metadata)?.buttons"
                    class="mr-auto text-sm text-[#00a5f4] flex flex-col relative max-w-[25em]">
                    <div v-for="(item, index) in JSON.parse(content.metadata)?.buttons" :key="index"
                        class="flex justify-center items-center space-x-2 rounded-lg bg-white h-10 my-[0.1em]">
            ...
            <div v-if="props.type === 'outbound' && content.user" class="mt-2 mb--2">
                <span class="text-gray-500 text-xs text-right leading-none">Sent By: <u>{{ content.user?.first_name + `
                        ` + content.user?.last_name }}</u></span>
            </div>
...
const getErrors = (logIndex = 0) => {
    try {
        if (!props.content?.logs?.[logIndex]?.metadata) return [];
        const metadata = JSON.parse(props.content.logs[logIndex].metadata);
        return metadata.errors || [];
    } catch (error) {
        console.error("Error parsing metadata:", error);
...
<template>
    <div class="flex mb-2" :class="props.type === 'outbound' ? 'justify-end' : 'justify-start'">
        <div class="max-w-[85%] md:max-w-[70%]">
            <div class="rounded-2xl px-4 py-2 shadow-sm" :class="props.type === 'outbound'
                ? 'bg-[#d8fad4] rounded-tr-none'
                : 'bg-white rounded-tl-none'">

                <!-- Text Message -->
                <div v-if="JSON.parse(content.metadata).type === 'text'">
                    <div v-if="JSON.parse(content.metadata).header?.text" class="font-semibold text-gray-900 mb-1">
                    ...
                    <div v-if="JSON.parse(content.metadata)?.buttons" class="mt-3 space-y-2">
                        <div v-for="(item, index) in JSON.parse(content.metadata)?.buttons" :key="index"
                            class="flex items-center justify-center gap-2 px-4 py-2.5 bg-white hover:bg-gray-50 border border-gray-200 rounded-xl text-sm font-medium text-[#ff5100] transition-colors cursor-pointer">
                            <svg v-if="item.type === 'COPY_CODE'" xmlns="http://www.w3.org/2000/svg" width="16"
                                height="16" viewBox="0 0 24 24">
                                <path fill="currentColor"
                                    d="M19 21H8V7h11m0-2H8a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h11a2 2 0 0 0 2-2V7a2 2 0 0 0-2-2m-3-4H4a2 2 0 0 0-2 2v14h2V3h12V1Z" />
                            </svg>
                            <svg v-else-if="item.type === 'PHONE_NUMBER'" xmlns="http://www.w3.org/2000/svg" width="16"
                                height="16" viewBox="0 0 24 24">
                                <path fill="currentColor"
                            ...
                            <svg v-else-if="item.type === 'URL'" xmlns="http://www.w3.org/2000/svg" width="16"
                                height="16" viewBox="0 0 24 24">
                                <path fill="currentColor"
                ...
                <div v-if="JSON.parse(content.metadata).type === 'unsupported'"
                    ...
                    <div>
                        <p class="font-medium">{{ $t('Error code') }}: {{ JSON.parse(content.metadata).errors[0].code }}
                        </p>
                        <p class="text-sm mt-1">{{ JSON.parse(content.metadata).errors[0].error_data.details }}</p>
                    </div>
                </div>

                <!-- Button Reply -->
                <div v-if="JSON.parse(content.metadata).type === 'button'">
                    <p class="text-gray-800">{{ JSON.parse(content.metadata).button.text }}</p>
                </div>

                <!-- Interactive -->
                <div v-if="JSON.parse(content.metadata).type === 'interactive'">
                    <p v-if="JSON.parse(content.metadata).interactive.type == 'button_reply'" class="text-gray-800">
                        {{ JSON.parse(content.metadata).interactive?.button_reply?.title }}
                    </p>
                    <div v-if="JSON.parse(content.metadata).interactive.type == 'list_reply'">
                ...
                <div v-else-if="JSON.parse(content.metadata).type === 'image'">
                    <img v-if="content.media != null" :src="content?.media?.path" alt="Image"
                        class="rounded-xl max-w-full h-auto" style="max-width: 300px;" />
                    <div v-else class="flex items-center gap-2 text-gray-500 py-4">
                        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24">
                            <path fill="currentColor"
                                d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10s10-4.48 10-10S17.52 2 12 2zm1 15h-2v-2h2v2zm0-4h-2V7h2v6z" />
                        </svg>
                    ...
                    <p v-if="JSON.parse(content.metadata).image?.caption" class="mt-2 text-gray-700">
                        {{ JSON.parse(content.metadata).image?.caption }}
                    </p>
                </div>

                <!-- Document -->
                <div v-else-if="JSON.parse(content.metadata).type === 'document'" class="bg-gray-50 rounded-xl p-3">
                    <div class="flex items-center gap-3">
                        <div
                            class="flex-shrink-0 w-10 h-10 bg-[#ff5100]/10 rounded-lg flex items-center justify-center">
                            <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24"
                                class="text-[#ff5100]">
                                <path fill="currentColor"
                        ...
                        <div class="flex-1 min-w-0">
                            <p class="text-sm font-medium text-gray-900 truncate">{{ content?.media?.name }}</p>
                            <p class="text-xs text-gray-500">{{ getExtension(content?.media?.type) }} • {{
                                formatFileSize(content?.media?.size) }}</p>
                        </div>
                        <a :href="content?.media?.path" :download="content?.media?.name" target="_blank"
                            @click="downloadClicked"
                            class="flex-shrink-0 p-2 hover:bg-gray-200 rounded-lg transition-colors">
                            <svg v-if="!downloading" xmlns="http://www.w3.org/2000/svg" width="20" height="20"
                ...
                <!-- Location -->
                <div v-else-if="JSON.parse(content.metadata).type === 'location'" class="rounded-xl overflow-hidden">
                    <GoogleMap :api-key="getValueByKey('google_maps_api_key')" style="width: 300px; height: 200px"
                        :center="location(content.metadata)" :zoom="15">
                        <Marker :options="{ position: location(content.metadata) }" />
                    </GoogleMap>
                ...
                <!-- Sticker -->
                <div v-else-if="JSON.parse(content.metadata).type === 'sticker'">
                    <img v-if="content.media != null" :src="content?.media?.path" alt="Sticker" class="w-24 h-24" />
                    <div v-else class="flex items-center gap-2 text-gray-500 py-4">
                        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24">
                            <path fill="currentColor"
                                d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10s10-4.48 10-10S17.52 2 12 2zm1 15h-2v-2h2v2zm0-4h-2V7h2v6z" />
                ...
                <!-- Contacts -->
                <div v-else-if="JSON.parse(content.metadata).type === 'contacts'" class="bg-gray-50 rounded-xl p-3">
                    <div class="flex items-center gap-3">
                        <div class="w-10 h-10 rounded-full bg-[#ff5100]/10 flex items-center justify-center">
                        ...
                        <p class="text-sm font-medium text-gray-900">{{ getContactDisplayName(content.metadata) }}</p>
                    </div>
                </div>

                <!-- Audio -->
                <div v-else-if="JSON.parse(content.metadata).type === 'audio'">
                    <audio v-if="content.media != null" controls class="w-full max-w-sm">
                        <source :src="content?.media?.path">
                        {{ $t('Your browser does not support the audio element') }}
                    </audio>
                    <div v-else class="flex items-center gap-2 text-gray-500 py-4">
                ...
                <!-- Video -->
                <div v-else-if="JSON.parse(content.metadata).type === 'video'">
                    <video v-if="content.media != null" controls class="rounded-xl max-w-full"
                    ...
                    <p v-if="JSON.parse(content.metadata).video?.caption" class="mt-2 text-gray-700">
                ...
                <div class="flex items-center justify-between gap-2 mt-2">
                    <div class="text-xs text-gray-600">
                        <span v-if="props.type === 'outbound' && content.user" class="mr-2">
                            <span class="font-medium">{{ content.user?.first_name }}</span>
                        </span>
                        <span>{{ content.created_at }}</span>
                    </div>

                    <!-- Status Icons -->
                    <div v-if="props.type === 'outbound'" class="flex-shrink-0">
                        <div class="group relative cursor-pointer">
                            <div
                                class="absolute bottom-full mb-1 hidden group-hover:block bg-gray-800 text-white text-xs rounded px-2 py-1 whitespace-nowrap">
                                {{ chatStatus(content.logs) }}
                            </div>

                            <!-- Sent -->
                            <svg v-if="chatStatus(content.logs) === 'sent'" xmlns="http://www.w3.org/2000/svg"
                                width="16" height="16" viewBox="0 0 16 16" class="text-gray-500">
                                <path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"
                                    stroke-width="1.5" d="m2.75 8.75l3.5 3.5l7-7.5" />
                            </svg>
                            ...
                            <!-- Failed -->
                            <svg v-if="chatStatus(content.logs) === 'failed'" @click="isModalOpen = true"
                                xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 16 16"
                                class="text-red-600">
                                <path fill="currentColor"
                                    d="M8 1a7 7 0 1 0 0 14A7 7 0 0 0 8 1ZM2 8a6 6 0 1 1 12 0A6 6 0 0 1 2 8Zm6-3a.5.5 0 0 1 .5.5v3a.5.5 0 0 1-1 0v-3A.5.5 0 0 1 8 5Zm0 6a.75.75 0 1 0 0-1.5a.75.75 0 0 0 0 1.5Z" />
                            </svg>
                        </div>
                    </div>
                </div>
    ...
    <Modal :label="$t('Message status: ') + chatStatus(content.logs)" :isOpen="isModalOpen" :closeBtn="true"
        ...
        <div v-if="errors.length" class="mt-4 space-y-3">
            <div v-for="(error, index) in errors" :key="index"
                class="bg-red-50 border border-red-200 rounded-xl p-4 overflow-x-auto">
                <div class="flex items-start gap-3 mb-3">
                    ...
                    <div class="flex-1">
                        ...
                        <div class="space-y-1 text-sm text-red-800">
                            ...
                            <p><span class="font-medium !text-wrap">Details:</span> {{ error.error_data.details }}</p>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div class="mt-6">
            <button type="button" @click="isModalOpen = false"
                class="w-full px-4 py-3 bg-gray-100 hover:bg-gray-200 rounded-xl text-sm font-medium text-gray-700 transition-colors">
                {{ $t('Close') }}
            </button>
        </div>
    </Modal>
